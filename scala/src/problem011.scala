...
object problem11 {
  ...
  def main(args: Array[String]): Unit = {
    val l = getGrid()
    val hors = getHors(l)
    val verts = getVerts(l)
    val diagL2R = getDiagLeftToRight(l)
    val diagR2L = getDiagRightToLeft(l)
    print(List(verts.max, hors.max, diagL2R.max, diagR2L.max).max)
  }

  def getGrid(): List[String] = {
  ...
  def getVerts(l: List[String]): List[Int] = {
    val m = new HashMap[Int, ListBuffer[Int]]
    for (a <- 0 until l.size) {
      val il = asIntList(l.apply(a))
      for (b <- 0 until il.size) {
        if (m.contains(b)) m.get(b).get += il.apply(b)
        else m.put(b, ListBuffer(il.apply(b)))
      }
    }
    val lb = new ListBuffer[Int]
    m.values.foreach { x => lb ++= sums(x.toList) }
    lb.toList
  }

  def getDiagRightToLeft(l: List[String]): List[Int] = {
    val m = new HashMap[String, ListBuffer[Int]]
    for (a <- 0 until l.size) {
      val il = asIntList(l.apply(a))
      for (b <- 0 until il.size) {
        val key = (a+b).toString
        if (m.contains(key)) m.get(key).get += il.apply(b)
        else m.put(key, ListBuffer(il.apply(b)))
      }
    }
    val lb = new ListBuffer[Int]
    m.values.foreach { x => lb ++= sums(x.toList) }
  ...
  def getDiagLeftToRight(l: List[String]): List[Int] = {
    val m = new HashMap[String, ListBuffer[Int]]
    for (a <- 0 until l.size) {
      val il = asIntList(l.apply(a))
      for (b <- 0 until il.size) {
  ...
  def asIntList(s: String): List[Int] = {
    s.split(" ").toList.map { x => x.toInt }
  }

  def getHors(l: List[String]): List[Int] = {
    val lb = new ListBuffer[Int]
    l.foreach { x => lb ++= sums(asIntList(x)) }
    lb.toList
  }
  
  def sums(l: List[Int]): List[Int] = {
    if (l.size < 4) List()
    else if (l.size == 4) List(l.product)
    else l.take(4).product :: sums(l.tail)
  }
